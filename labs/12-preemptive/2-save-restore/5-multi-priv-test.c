...
static inline uint32_t random_mode(void) {
    uint32_t modes[] = { 
        SYS_MODE,   // 0
        IRQ_MODE,   // 1
        SUPER_MODE, // 2
...
static void check_spsr(uint32_t expected_mode) {
    ...
    if(spsr_mode != expected_mode)
        panic("expected mode=%s: have %s\n", 
                mode_str(expected_mode), 
                mode_str(spsr_mode));
    trace("match fault from mode %s\n",
                mode_str(spsr_mode));
}
...
static void mutate_and_jump(void) {
    ...
    regs[16] = expected_regs[16] = random_mode();
...
void prefetch_abort_handler(uint32_t regs[17]) {
    ...
    check_spsr(mode_get(expected_regs[16]));
