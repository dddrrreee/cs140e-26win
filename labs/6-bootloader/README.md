...
You can often dramatically reduce complexity by not sharing stuff: running
one single thread versus many, one car on an empty highway versus rush
hour, etc.  This lab is no different.  If you have a a second microSD
card, you can use it to make it very easy to tell if you have a hardware
bug versus a bootloader issue.

Algorithm:
  1. Set aside the clean, working microSD you are currently using.
  2. Setup a second microSD that you will write all pi-side modifications
     to.  (Make sure you can boot from this before proceeding!)  Leave the
     first "golden copy" microSD alone.

  3. If your pi stops working during the lab, you can quickly do
     differential analysis to figure out the cause: swap back to the
     original microSD (with the original bootloader) and rerun.  
        - If it does work, you have a pi-side issue.
        - If it does not work, you either have a hardware issue or
          your unix bootloader is broken (run `pi-install`).
...
well of course!   Trying to debug while there are two errors multiplying
the causes will really drive home the reason you always want to isolate
and break things back town to a working system and then build back up.)
...
The r/pi miniUART hardware only has space to hold 8 received bytes.  If your
...
There's a bunch of useful [EXTENSIONS](EXTENSIONS.md).  

We've inlined two interesting ones below.

#### Extension: speed, Speed, *SPEED*.

Our default pi-to-laptop speed of 115,000 baud is *slow*.  We picked it
because there's usually a wide range of laptops in class and its slow
enough that it seems to work correctly for everyone.  However, if your
laptop can go fast seeing *how fast* is interesting.  You do do this
several ways:
  1. Low-tech: Simply extend the current bootloader to have a hard-wired
     faster speed.  This is pretty simple, but requires a bunch of
     binary-search-ish cycles to figure a reliable number.  (We will check
     in a `uart_init` that lets you set different speeds.)
  2. Have an adaptive protocol.  You can do a simple extension to have the
     pi side send a proposed speed and have the laptop reply with its
     offered speed.  Of course, a danger is that because this message isn't 
     in our original protocol that it will cause the original `pi-install`
     to fail.  Fortunately, because the unix side discards initial bytes
     it does not recognize, it will just discard this and continue.  Nice.
  3. Make your own crazy protocol, hyper-optimized for sending bytes. You'll
...
solid, even under weird errors.  The booloader is the lowest part of the
system, so any bug will both effect the most programs and (generally)
be the hardest to debug.
