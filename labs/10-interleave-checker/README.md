...
which cause a fault when the progam counter is set to a specified
address.  These are usually used by debuggers to run a debugged program
at full-speed until a specific location is read, written or executed.
While useful, empirically this ability doesn't sound very exciting; and
most kernel hackers I've known simply skip the part of the architecture
manual that talks about debugging hardware.

However, what no one seems to realize is that because this debug hardware
can be used to dynamically see:
  - *every* instruction as it executes;
  - *every* register value while it does so;
they make it very easy to write dynamic analysis tools in a couple
hundred lines of code that can find subtle, nasty bugs in your kernel.
Bugs that otherwise can take days, weeks or, likely, never to diagnose
can instead be done in seconds.

In constrast, a traditionaly dynamic binary analysis tool such as Pin or
Valgrind are hundreds of thousands of lines of code at best, and can't
run in the kernel.

I think this is a fantastic hack, and you can do all sorts of cute tricks
with it.

So today we will do one.  And then next week we will do another (single
step equivalance) that will make it easy to verify that user programs
behave identically as you add pre-emption, virtual memory, caching,
and other things.

The first part of the lab is a tutorial where you'll see examples of how
to use the interface.  (The next lab you'll build it.)  The bulk of the
lab is use it to for something cool: writing a concurrency checker that
is tiny but mighty.  By the end you'll be able to race condition bugs
difficult to catch with any other tool I know of.
...
in 240lx and last year's 140E, so hopefully is a fun one.  It's also
...
Checkoff:
 0. make sure you have the same hashes as other people for the 
    routines called by `1-many-fn-examples.c`
 1. Do the interleave checker and make sure you get the same results as
    us, roughly for the interleave checker.
...
As you recall from the background, single-stepping on the ARMv6 cpu we use
