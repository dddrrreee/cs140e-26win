...
uint16_t w5500_send_ping(const w5500_t* nic, const uint8_t* dest_ipv4_addr, const void* data, uint16_t nbytes, uint8_t socket) {
    ...
    *(cksum_ptr + 1) = (checksum & 0xFF);
    ...
    return w5500_write_ipv4_packet(nic, dest_ipv4_addr, PROTOCOL_ICMP, &icmp, icmp_length, socket);
}
...
uint16_t w5500_write_ipv4_packet(const w5500_t* nic, const uint8_t* dest_ipv4_addr, uint8_t ipv4_protocol, const void* data, uint16_t nbytes, uint8_t socket) {
    ...
    memcpy(packet.src_ip_address, nic->ipv4_addr, 4);
    ...
    *(cksum_ptr + 1) = (checksum & 0xFF);
    ...
    // ---------- Make frame! ---------- 
    return w5500_write_frame(nic, dest_hw_addr, FRAME_IPV4, &packet, packet_length, socket);
}
...
uint16_t w5500_write_frame(const w5500_t* nic, const uint8_t* dest_hw_addr, uint16_t ethertype, void* data, uint16_t nbytes, uint8_t socket) {
    ...
    memcpy(frame.src_hw_addr, nic->hw_addr, 6);
    ...
    return w5500_write_tx_bytes(nic, &frame, frame_length, socket);
}

// uint16_t w5500_write_frame(const w5500_t* nic, const frame_t* frame, uint16_t data_len, uint8_t socket) {
//     uint16_t frame_length = data_len + FRAME_HEADER_BYTES;
//     frame_t buffer;
//     memcpy(&buffer, frame, frame_length);
//     swapEndian16(&buffer.ethertype); // Swap ethertype since it is sent big endian

//     // print_bytes("Buffer:", &buffer, frame_length);
     
//     return w5500_write_tx_bytes(nic, &buffer, frame_length, socket);
// }
